...
#line 1 "mycode\user.c" /0
 ...
 s8 cmp_pwm_result = 50;
 u8 screen_pwm_duty_limit = 0;
 
 void CLK_Init(void)
 {
  (P_SW2 |= (1<<7)); 
  (*(unsigned char volatile xdata *)0xfe00) = 0x00; 
 ...
 cmp_pwm_result_temp1 /= 5;
 
 if((cmp_pwm_result_temp1 < cmp_pwm_result_temp2 - 4)
 || (cmp_pwm_result_temp1 > cmp_pwm_result_temp2 + 4))
 {
 cmp_pwm_result = (1000 - cmp_pwm_result_temp1*10)/10;
 if(cmp_pwm_result > 100)
 {
 cmp_pwm_result = 100;
 }
 else if(cmp_pwm_result < 1)
 {
 cmp_pwm_result = 1;
 }
 ...
 if(screen_pwm_duty > 100)
 {
 screen_pwm_duty = 100;
 }
 else if(screen_pwm_duty < 1)
 {
 screen_pwm_duty = 1;
 }
 ...
 if(pwm_in_duty_cnt)
 {
 screen_pwm_duty_limit = pwm_in_duty_cnt;
 }
 pwm_in_duty_cnt = 0;
 }
 }
